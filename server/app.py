    ...
    with SessionLocal() as db:
        ...
        if active_count > 0:
            raise HTTPException(
                status_code=400,
                detail=(
                    "Cannot delete agent with active requests. "
                    f"Agent has {active_count} active request(s)"
                ),
            )
...
async def get_stats():
    with SessionLocal() as db:
        ...
        active_requests = db.query(RequestModel).filter(
            RequestModel.status == "processing"
        ).count()
        completed_requests = db.query(RequestModel).filter(
